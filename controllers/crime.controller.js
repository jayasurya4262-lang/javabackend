...
router.post("/", async (req, res) => {
  ...
  // Corrected: Use toLowerCase() for case-insensitive comparison
  if (crime.crimeType && crime.crimeType.toLowerCase() === "serial killer") {
    crime.priority = "HIGH"
  ...
  try {
    ...
    console.log("Crime saved successfully with ID:", savedCrime._id) // Mongoose uses _id
...
router.put("/:id", async (req, res) => {
  ...
  if (existingCrime) {
    crimeData._id = id // Ensure Mongoose uses _id for update
    const updatedCrime = await crimeService.saveCrime(crimeData) // saveCrime handles update if _id is present
    console.log("Crime updated successfully with ID:", updatedCrime._id) // Mongoose uses _id
